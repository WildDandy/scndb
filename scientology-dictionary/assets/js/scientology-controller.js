// Scientology Dictionary Controller
// Handles UI interactions and display logic

class ScientologyDictionaryController {
    constructor() {
    ...
    async init() {
        ...
        this.displayGlossaryItems(scientologyDictionaryData.data);
        this.updateCount(scientologyDictionaryData.data.length);
    }

    async waitForDataLoad() {
        while (!scientologyDictionaryData.isLoaded) {
    ...
    generateAlphabetNav() {
        ...
        const letters = scientologyDictionaryData.getAlphabetLetters();
    ...
    handleFilter() {
        ...
        if (this.currentFilter) {
            filteredData = scientologyDictionaryData.filterByQuery(this.currentFilter);
        } else if (this.currentLetter) {
            filteredData = scientologyDictionaryData.filterByLetter(this.currentLetter);
        } else {
            filteredData = scientologyDictionaryData.data;
        }
    ...
    updateCount(count = null) {
        ...
        const displayCount = count !== null ? count : scientologyDictionaryData.getFilteredCount();
        const totalCount = scientologyDictionaryData.getTotalCount();
...
document.addEventListener('DOMContentLoaded', () => {
    new ScientologyDictionaryController();
});
